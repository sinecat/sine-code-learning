## Todo-List 案例
![image-20221206192902233](https://october-x-image-host.oss-cn-hangzhou.aliyuncs.com/markdown-imgsimage-20221206192902233.png)
**组件化编码流程**

1. 拆分静态组件:组件要按照功能点拆分，命名不要与`html` 元素冲突
2. 实现动态组件:考虑好数据的存放位置，数据是一个组件在用，还是一些
    组件在用
    - —个组件在用:放在组件自身即可
...
3. 实现交互:从绑定事件开始
    ...
    - 子组件==>父组件通信（要求父组件先给子组件一个函数)
        使用`v-model` 时要切记:`v-model `绑定的值不能是`props` 传过来的值，因为`props` 是不可以修改的`props` 传过来的若是对象类型的值，修改对象中的属性时`Vue`不会报错，但不推荐这样做

## **总结**

遵守原则：`props`是只读的，谁的数据由谁改。
实现儿子给父亲传东西，父亲给儿子传一个函数，儿子调用，父亲可以收到参数。在函数中实现数据更新的操作。当然可以是爷爷传给孙子。

...后续还有使用组件自定义事件实现
**nanoid**：
使用`nanoid`实现生成不重复的id值
`jgu_nif_avSTxmIcbGIzh`
...
```
![image-20221206193136718](https://october-x-image-host.oss-cn-hangzhou.aliyuncs.com/markdown-imgsimage-20221206193136718.png)
