...
[Tutorial v6.4.3 | React Router](https://reactrouter.com/en/main/start/tutorial)

将官方文档的教程案例熟悉一下，里面有用loader和action理解一下。
...
3. `<Route>` 相当于一个 if 语句，如果其路径与当前 URL 匹配，则呈现其对应的组件，不再往下匹配。
...
2. `replace`属性用于控制跳转模式（push（配置为false） 或 replace（配置为true），默认是push）。
...
参考：[教程 v6.4.3 |反应路由器 (reactrouter.com)](https://reactrouter.com/en/main/start/tutorial)
...
参考官方文档[Home v6.4.3 | React Router](https://reactrouter.com/en/main)，里面还有很多知识。
