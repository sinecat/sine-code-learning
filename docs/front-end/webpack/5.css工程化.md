...
module.exports = {}
//这样我们就不需要自己引用了。style-loader会自动完成。
```
...
![](https://october-x-image-host.oss-cn-hangzhou.aliyuncs.com/markdown-imgs/2020-01-31-09-53-31.png)
...
![](https://october-x-image-host.oss-cn-hangzhou.aliyuncs.com/markdown-imgs/2020-01-31-13-54-37.png)
...
rules: [
            {
                ...
                //     options: {
                //         // modules: {
                //         //     localIdentName: "[local]-[hash:5]"//样式模块名+hash值
                //         // }
                //         modules:true
                //     }
                // }]
...
![](https://october-x-image-host.oss-cn-hangzhou.aliyuncs.com/markdown-imgs/2020-01-31-14-00-56.png)

原理极其简单，开启了css module后，css-loader会将样式中的类名进行转换，转换为一个唯一的hash值。

由于hash值是根据模块路径和类名生成的，因此，不同的css模块，哪怕具有相同的类名，转换后的hash值也不一样。

![](https://october-x-image-host.oss-cn-hangzhou.aliyuncs.com/markdown-imgs/2020-01-31-14-04-11.png)
...
![](https://october-x-image-host.oss-cn-hangzhou.aliyuncs.com/markdown-imgs/2020-01-31-14-08-49.png)
...
![](https://october-x-image-host.oss-cn-hangzhou.aliyuncs.com/markdown-imgs/2020-02-03-11-48-45.png)

目前，最流行的预编译器有**LESS**和**SASS**，由于它们两者特别相似，因此仅学习一种即可（本课程学习LESS）

![](https://october-x-image-host.oss-cn-hangzhou.aliyuncs.com/markdown-imgs/2020-02-03-11-50-05.png)
...
module.exports = {
    ...
    module: {
        rules: [
            { test: /\.css$/, use: ["style-loader", "css-loader"] },
            //加上less的配置，才会以less的方式处理，否则以js的方式导致报错
            //注意执行顺序是右到左的
            //考虑到要和正常的bem和css module连用，主要是和后者连用，所以后面加上modules
...
![](https://october-x-image-host.oss-cn-hangzhou.aliyuncs.com/markdown-imgsimage-20221030153924264.png)

看上去是不是和LESS、SASS一样呢？

但PostCss和LESS、SASS的思路不同，它其实只做一些代码分析之类的事情，将分析的结果交给插件，具体的代码转换操作是插件去完成的。类似于webpack的流程。

![](https://october-x-image-host.oss-cn-hangzhou.aliyuncs.com/markdown-imgsimage-20221030153942111.png)

官方的一张图更能说明postcss的处理流程：

![](https://october-x-image-host.oss-cn-hangzhou.aliyuncs.com/markdown-imgsimage-20221030154001003.png)
...
```js
//以代码的形式
test('css代码',function(content)){
     ...
     return centent.css//编译后的代码
}
```
...
`postcss`的源文件可以是`.css`、`.postcss`、`.pcss`

记得安装·`PostCSS Language Support`方便代码的编写
...
插件也可以直接在npm网站搜索：https://www.npmjs.com/
...
![](https://october-x-image-host.oss-cn-hangzhou.aliyuncs.com/markdown-imgsimage-20221030161928513.png)
...
设置为`null`可以禁用该规则，或者设置为4，表示一个缩进有4个空格。具体的设置需要参见stylelint文档：

https://stylelint.io/

但是这种错误报告需要在编译时才会发生，并且也会生成最终di如果我希望在编写代码时就自动在编辑器里报错呢？
        ...
        rules: [
            {
                ...
                //写法也可以是css-loader中的options设置modules为true
            }
        ]
    },
    devServer: {
